...
import { CubeRatesDataService } from './src/potential-cost/data/cube-rates-data.service';
import {
  PotentialTier,
  CubeType,
  ItemType,
...
async function testPotentialCalculation() {
  ...
  try {
    ...
    const potentialService = new PotentialService(calculationService, costService);

    // Test parameters: Item Type Shoes, red cubes, epic tier, 12% Stat
    const testRequest = {
      desiredTier: PotentialTier.EPIC,
      probabilityInput: {
        percStat: 12, // At least 12% stat
        lineStat: 0,
        percAllStat: 0,
        lineAllStat: 0,
        percHp: 0,
      ...
      itemLevel: 160,
      currentTier: PotentialTier.RARE,
      isDMT: false,
    };
    ...
    console.log(`   Current Tier: ${testRequest.currentTier}`);
    console.log(`   Desired Tier: ${testRequest.desiredTier}`);
    console.log(`   Goal: At least 12% stat`);
    console.log('');

    // Get cube rates data
    const cubeRates = cubeRatesDataService.getCubeRates();
    console.log('✅ Cube rates data loaded successfully');

    // Test the calculation
    console.log('🔬 Running calculation...');
    const result = potentialService.calculatePotential(testRequest, cubeRates);
